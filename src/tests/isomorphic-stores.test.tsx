...
test("useStore outside provider throws", () => {
  ...
  function Widget() {
    const x = SomeStore.useStore(s => s.x);
    return <div>{x}</div>;
  }
  ...
  console.error = () => {};
  try {
    expect(() => render(<Widget />)).toThrow("isomorphic-stores: cannot call useStore outside a provider");
  } finally {
...
test("waitFor: initial value shown before promise resolves", async () => {
  ...
  function Widget() {
    const name = NameStore.useStore(s => s.name);
    return <div>{name}</div>;
  }
...
test("waitFor: rejection triggers onError, keeps initial value, whenReady still resolves", async () => {
  ...
  function Widget() {
    const name = FailStore.useStore(s => s.name);
    return <div>{name}</div>;
  }
...
test("clientOnly: initial value shown before mount; resolves after mount", async () => {
  ...
  function Widget() {
    const data = DataStore.useStore(s => s.data);
    return <div>{data}</div>;
  }
...
test("useCreateClientStore: undefined before ready, value after", async () => {
  const SimpleStore = defineZustandIsoStore<{}, { value: string }>(
    () => () => ({ value: "hello" })
  );

  function Widget() {
    const { ready, useClientStore } = SimpleStore.useCreateClientStore({});
    const value = useClientStore(s => s.value);
...
test("broadcast delivers message to all mounted instances", async () => {
  ...
  render(
    <>
      <IsoStoreProvider instances={[store1]}><Widget label="w1" /></IsoStoreProvider>
      <IsoStoreProvider instances={[store2]}><Widget label="w2" /></IsoStoreProvider>
    </>
  );
...
test("IsoStoreProvider wires multiple stores into the same tree", () => {
  ...
  function Widget() {
    const a = StoreA.useStore(s => s.a);
    const b = StoreB.useStore(s => s.b);
    return <div>{a} {b}</div>;
  }

  render(
    <IsoStoreProvider instances={[storeA, storeB]}>
      <Widget />
    </IsoStoreProvider>
  );
